...
public class StudentSwaggerconfig {
	...
	public Docket postsApi() {
		return new Docket(DocumentationType.SWAGGER_2).apiInfo(metadata()).select().paths(regex("/clg-rest.*")).build();
	}

	@SuppressWarnings("deprecation")
	private ApiInfo metadata() {
		return new ApiInfoBuilder().title("Java task assignment_Togglenow").description("Java task assignment_Togglenow").termsOfServiceUrl("https://www.togglenow.com/").contact("subbu, chaetiyar").version("1.0").build();	
	}
}